        ...
        <div class="container unwind-wallet" id="container">
            <div class="account-index-wrapper">
                <div class="unwind-info">
                    ...
                    <div class="warm-note">如您销户成功，则无法使用账号{{showmob}}，登录钱包账户</div>
                </div>
                <button class="send-code" :class="[actsend ? '' : 'actsend-code']" @click="getcode">发送验证码</button>
...
export default {
    ...
    created() {
        this.querydataList = window.sessionStorage.getItem("saveCancelQuery") || ''
        var data = {}
        if(this.querydataList) {
            this.querydataList = JSON.parse(this.querydataList)
            data = this.querydataList
        }
        ...
        this.livingOrderNo = this.$route.query.livingOrderNo || data.livingOrderNo || "";
        this.orgCode = this.$route.query.orgCode || data.orgcode || "";
        this.queryCrpmobile()
    },
    methods: {
        queryCrpmobile() {
                ...
                .then(data => {
                    data.list.forEach(res => {
                        if(res.orgCode == "BOCM") {
                            ...
                            this.crpNum = res.crpmobile.replace(/(\d{3})\d{4}(\d{4})/, "$1****$2")
                        }
                    })
                })
            } else if(JSON.parse(window.sessionStorage.getItem("crpMobile"))) {
                this.crpNum = JSON.parse(window.sessionStorage.getItem("crpMobile")).replace(/(\d{3})\d{4}(\d{4})/, "$1****$2");
            }
        },
        getcode() {
            ...
            if(this.$route.query.orgCode) {
                bankcode = this.$route.query.orgCode.toUpperCase()
            } else if(this.querydataList) {
                bankcode = this.querydataList.orgcode
            }
            api.cancelAccSms({
                mobileNo: this.mobile, // this.mobile13814088104
                orgCode: bankcode,
                livingOrderNo: this.livingOrderNo,
            })
                .then((data) => {
                    console.log(data);
                    this.userDetermine();
        ...
        checkInfor() {
            ...
            if (this.smsCode.length === 6) {
                api.cancelAccValiCode({
                    ...
                    .then((data) => {
                        console.log(data);
                        store2.default.set("signAlready",false)
                    ...
                    .catch((err) => {
                        ...
                        if (err.msgCd === 'BUS20017') {
                            ...
                            if (err.msgInfo.indexOf('交易密码连续输错超过三次') > -1) {
                                this.$router.push(
                                    {
                                        name: 'cancellResult',
                                        query: {
                                            result: 1,
                                            msg: '因您曾在浦发银行重置过交易密码，暂无法在掌厅销户，请前往浦发银行网点销户'
                                        }
                                    }
                                );
                            } else {
                            ...
                            //     this.entertimes += 1;
                            //     console.log(this.entertimes);
                            //     this.$toast.info('您的验证输入有误，请重新输入');
                            // } else {
                            //     this.$router.push(
                            //         {
                            //             name: 'cancellResult',
    ...
    destroyed() {
        window.sessionStorage.setItem("saveCancelQuery",'')
    }
};
</script>
